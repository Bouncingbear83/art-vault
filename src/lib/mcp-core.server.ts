// =====================================================================
// ART360 :: MCP core (JSON-RPC 2.0 dispatcher + tools)
// Shared by /api/public/mcp and /api/public/mcp/$key.
// Service role bypasses RLS; callers must be authorised by the route.
// =====================================================================
...
export async function dispatch(msg: any): Promise<any | null> {
...
// Deliberately no WWW-Authenticate header: it pushes MCP clients into an
// OAuth discovery/registration flow this server does not implement.
...
/** Handles an authorised JSON-RPC POST (single message or batch). */
export async function handleRpcPost(request: Request): Promise<Response> {
  let body: any;
  try {
